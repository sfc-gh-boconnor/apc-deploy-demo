    ...
    if (shiftFrom && shiftTo) {
      const rebalanceResults = await querySnowflake(`
        SELECT
          ...
          src.COGM_PER_UNIT AS FROM_COGM,
          dst.PLANT_CODE AS TO_PLANT,
          dst.COGM_PER_UNIT AS TO_COGM,
          ROUND(src.COGM_PER_UNIT - dst.COGM_PER_UNIT, 2) AS SAVINGS_PER_UNIT,
          ROUND((src.COGM_PER_UNIT - dst.COGM_PER_UNIT) / NULLIF(src.COGM_PER_UNIT, 0) * 100, 1) AS SAVINGS_PCT
